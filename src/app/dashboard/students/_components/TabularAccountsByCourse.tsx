...
interface TabularAccountsByCourseProps {
...
export const TabularAccountsByCourse: React.FC<
  TabularAccountsByCourseProps
> = ({ courseShortName }) => {
  ...
  const filteredAccounts = accountsData.accounts.filter((account) => {
    const fullName =
      `${account.last_name} ${account.first_name} ${account.middle_name}`.toLowerCase();
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {fullCourseName} - ({accountsData.account_count})
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="mb-4">
                ...
                <TableCell className="py-2 pr-[-20px]">
                  <StudentLink schoolId={account.school_id}>
                    <img
                      width="24"
                      src={
                        account.cropped_avatar_url || "/images/def-avatar.svg"
                      }
                      alt={
                        account.cropped_avatar_url ? "avatar" : "default avatar"
                      }
                    />
                  </StudentLink>
                </TableCell>
...
export default TabularAccountsByCourse;
